...
def translate_text(text, src_lang, tgt_lang):
    ...
    if model is None or tokenizer is None:
        return translate_large_text_google(text, src_lang, tgt_lang)

    chunks = textwrap.wrap(text, width=MAX_CHAR_LIMIT, break_long_words=False)
    translated_chunks = []
    
    for chunk in chunks:
        tokenized_text = tokenizer(chunk, return_tensors="pt", padding=True, truncation=True)
...
def translate_large_text_google(text, src_lang, tgt_lang):
    chunks = textwrap.wrap(text, width=MAX_CHAR_LIMIT, break_long_words=False)
    translated_chunks = [google_translate(chunk, src_lang, tgt_lang) for chunk in chunks]
